...
//			  }
			...
			$status         = 1; $nm_hak_akses   = $this->managePermission; $kode_universal = $tahun; $jumlah = $x;
...
//			  }
	...
	function approve_data($tahun)
    {
        if($tahun!=''){
            ...
            $status         = 1; $nm_hak_akses   = $this->deletePermission; $kode_universal = $tahun; $jumlah = 1;
            $sql            = $this->db->last_query();
			$result=1;
        } else {
            $result = 0;
            $keterangan     = "GAGAL, approve data Budget ".$tahun;
            $status         = 0; $nm_hak_akses   = $this->deletePermission; $kode_universal = $tahun; $jumlah = 1;
            $sql            = $this->db->last_query();
        }
		simpan_aktifitas($nm_hak_akses, $kode_universal, $keterangan, $jumlah, $sql, $status);
        $param = array(
	...
	function create_budget_umum(){
		...
		if(empty($datcoa)){
			echo 'Data tidak ditemukan <a href="'.base_url('budget_coa/budget_nr').'">Kembali</a>';die();
