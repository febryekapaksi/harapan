...
<head>
    ...
    <title>Report Hutang Per Invoice</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        thead tr th {
            ...
            text-align: center; border: 1px solid #999; font-size: 10px;
        }
        tbody tr td {
        ...
        tfoot tr td {
            ...
            font-weight: bold; font-size: 11px; background: #eaf2fb;
        }
...
<div class="page-wrapper">
    ...
    <div class="report-header">
        <h2>Report Hutang Per Invoice</h2>
    ...
    <div class="info-bar">
        ...
        <div class="total-box">
            Total Hutang: Rp <?= number_format($total_hutang, 0, ',', '.') ?>
        </div>
    </div>
    ...
    <table>
        ...
        <?php else: ?>
            ...
            function fmt_tgl_hutang($d, $months_id) {
                if (empty($d)) return '';
                $ts = strtotime($d);
                return date('d', $ts) . ' ' . $months_id[(int)date('n', $ts)] . ' ' . date('Y', $ts);
            }
            function fmt_num_hutang($n) {
                if ($n === '' || $n === null) return '';
            ...
            <tr>
                ...
                <td rowspan="<?= $row['rowspan'] ?>"><?= htmlspecialchars($row['nm_supplier']) ?></td>
                <td rowspan="<?= $row['rowspan'] ?>" class="text-center"><?= fmt_tgl_hutang($row['tgl_invoice'], $months_id) ?></td>
                <td rowspan="<?= $row['rowspan'] ?>" class="text-center"><?= htmlspecialchars($row['no_po']) ?></td>
                <td rowspan="<?= $row['rowspan'] ?>" class="text-center"><?= htmlspecialchars($row['id_invoice']) ?></td>
                <td rowspan="<?= $row['rowspan'] ?>" class="text-right"><?= fmt_num_hutang($row['nilai_invoice']) ?></td>
                <?php endif; ?>
                <td class="text-center"><?= htmlspecialchars($row['kd_pembayaran']) ?></td>
                <td class="text-center"><?= fmt_tgl_hutang($row['tgl_bayar'], $months_id) ?></td>
                <td class="text-right"><?= fmt_num_hutang($row['nilai_bayar']) ?></td>
                <td class="text-right"><?= fmt_num_hutang($row['total_bayar']) ?></td>
                <td class="text-right"><?= fmt_num_hutang($row['sisa_hutang']) ?></td>
            </tr>
            <?php endforeach; ?>
        <?php endif; ?>
        </tbody>
        <tfoot>
            <tr>
                <td colspan="9" class="text-right">Total Hutang</td>
