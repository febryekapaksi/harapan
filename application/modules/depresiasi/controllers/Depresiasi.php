...
class Depresiasi extends Admin_Controller
{
    protected $viewPermission   = 'Depresiasi.View';
    protected $addPermission    = 'Depresiasi.Add';
    protected $managePermission = 'Depresiasi.Manage';
    protected $deletePermission = 'Depresiasi.Delete';
    ...
    // -----------------------------------------------------------------------
    // INDEX – Daftar jadwal depresiasi bulanan
    // -----------------------------------------------------------------------
    public function index()
    {
        $this->auth->restrict($this->viewPermission);
        $this->template->title('Depresiasi Asset');
        ...
        history("View index depresiasi");
        $this->template->render('index', $dataArr);
    }

    // -----------------------------------------------------------------------
    // DATA SIDE – DataTables server-side untuk jadwal depresiasi
    // -----------------------------------------------------------------------
    public function data_side()
    {
        $this->Depresiasi_model->getDataJSON();
    }
    ...
    public function detail()
    {
        ...
        $data = $this->Depresiasi_model->getDetailDepresiasi($bulan, $tahun, $kdcab);
    ...
    public function preview_jurnal()
    {
        ...
        $cek_jurnal = $this->Depresiasi_model->cekJurnalBulan($bulan, $tahun, $kdcab);

        $detail = $this->Depresiasi_model->getDetailDepresiasi($bulan, $tahun, $kdcab);
        ...
        $this->load->view('Depresiasi/preview_jurnal', $dataArr);
    }
    ...
    public function proses_otomatis()
    {
        ...
        // curl -s "http://domain.com/depresiasi/proses_otomatis"
        $bulan = date('m');
        $tahun = date('Y');
    ...
    public function batal_jurnal()
    {
        ...
        $ArrNomor = $this->db->query(
            "SELECT DISTINCT nomor FROM " . DBACC . ".jurnal
             WHERE jenis_trans = 'depresiasi asset'
             AND MONTH(tanggal) = '" . (int)$bulan . "'
             AND YEAR(tanggal)  = '" . (int)$tahun . "'"
        )->result_array();

        if (empty($ArrNomor)) {
            echo json_encode(array('status' => 0, 'pesan' => 'Tidak ada jurnal depresiasi untuk periode ini.'));
            return;
        }
        ...
        $this->db->query("DELETE FROM " . DBACC . ".jurnal WHERE nomor IN " . $inNomor . " AND jenis_trans = 'depresiasi asset'");
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            echo json_encode(array('status' => 0, 'pesan' => 'Gagal membatalkan jurnal depresiasi.'));
        } else {
            $this->db->trans_commit();
            history("Batal jurnal depresiasi " . $bulan . "/" . $tahun);
            echo json_encode(array('status' => 1, 'pesan' => 'Jurnal depresiasi berhasil dibatalkan.'));
        }
    }
    ...
    {
        $this->auth->restrict($this->viewPermission);
        $this->template->title('Log Jurnal Depresiasi');
        history("View log jurnal depresiasi");
        $this->template->render('log_jurnal');
    }

    public function data_log()
    {
        $this->Depresiasi_model->getLogJSON();
    }

    // -----------------------------------------------------------------------
    // PRIVATE – inti proses jurnal depresiasi
    // -----------------------------------------------------------------------
    private function _proses_jurnal($bulan, $tahun, $user = 'System')
    {
        ...
        // Ambil data depresiasi bulan ini beserta COA dari category
        $ArrAsset = $this->Depresiasi_model->getDepresiasiUntukJurnal($bulan, $tahun, $kdcab);

        if (empty($ArrAsset)) {
            return array('status' => 0, 'pesan' => 'Tidak ada data depresiasi untuk periode ' . $bulan . '/' . $tahun . '.');
        }
        ...
        foreach ($ArrAsset as $valx) {
            ...
            $ketD = !empty($valx['nm_coa_debit'])  ? strtoupper($valx['nm_coa_debit'])  : 'BIAYA DEPRESIASI ASSET';
            $coaK = !empty($valx['coa_kredit'])   ? $valx['coa_kredit']   : '1309-00-00';
            $ketK = !empty($valx['nm_coa_kredit']) ? strtoupper($valx['nm_coa_kredit']) : 'AKUMULASI DEPRESIASI ASSET';
        ...
        $ArrJavh[] = array(
            ...
            'koreksi_no'    => '-',
            'jenis'         => 'JV',
            'keterangan'    => 'DEPRESIASI ASSET ' . $bulan . '/' . $tahun,
            'bulan'         => $bln_int,
            'tahun'         => $tahun,
            'user_id'       => $user,
        ...
        if (!empty($ArrNomorLama)) {
            ...
            $this->db->query("DELETE FROM " . DBACC . ".jurnal WHERE nomor IN " . $inLama . " AND jenis_trans = 'depresiasi asset'");
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->db->query(
                "INSERT INTO asset_jurnal_log (tanggal, ket, jurnal_by, bulan, tahun, kdcab)
                 VALUES ('" . date('Y-m-d H:i:s') . "', 'FAILED-DEPRESIASI', '" . $user . "', '" . $bulan . "', '" . $tahun . "', '" . $kdcab . "')"
            );
            return array('status' => 0, 'pesan' => 'Jurnal depresiasi gagal disimpan. Silakan coba lagi.');
        } else {
            $this->db->trans_commit();
            $this->db->query(
                "INSERT INTO asset_jurnal_log (tanggal, ket, jurnal_by, bulan, tahun, kdcab)
                 VALUES ('" . date('Y-m-d H:i:s') . "', 'SUCCESS-DEPRESIASI', '" . $user . "', '" . $bulan . "', '" . $tahun . "', '" . $kdcab . "')"
            );
            history("Posting jurnal depresiasi " . $bulan . "/" . $tahun);
            return array('status' => 1, 'pesan' => 'Jurnal depresiasi berhasil diposting untuk periode ' . $bulan . '/' . $tahun . '.');
        }
    }
}
