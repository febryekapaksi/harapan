...
class Setor_bank_model extends BF_Model
{
    ...
    {
        // Format tanggal setor jadi Ymd
        $prefix = 'SET' . date('Ymd', strtotime($tgl_setor));
    ...
    public function get_json_setoran_bank()
    {
        ...
        if (!empty($result_data)) {
            ...
            $details = $this->db->select('id_setor, GROUP_CONCAT(kd_pembayaran SEPARATOR ",") as kode_penerimaan')
                ->from('tr_setor_detail')
                ->where_in('id_setor', $ids)
                ->group_by('id_setor')
                ->get()->result_array();
            foreach ($details as $d) {
                $mapPenerimaan[$d['id_setor']] = $d['kode_penerimaan'];
            }
        }
