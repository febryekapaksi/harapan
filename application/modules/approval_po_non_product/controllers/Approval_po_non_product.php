...
class Approval_po_non_product extends Admin_Controller
{
  //Permission
  protected $viewPermission   = 'Approval_PO_Non_Product.View';
  protected $addPermission    = 'Approval_PO_Non_Product.Add';
  protected $managePermission = 'Approval_PO_Non_Product.Manage';
  protected $deletePermission = 'Approval_PO_Non_Product.Delete';
  ...
  public function __construct()
  {
    ...
    $this->load->model(array('Approval_po_non_product/Approval_po_non_product_model', 'Purchase_order/Pr_model'));
  ...
  {
    $this->Approval_po_non_product_model->data_side_approval_pr_material();
  }
  ...
  public function po_approval($no_po)
  {
    ...
    // $getitemso = $this->db->join('dt_trans_po_non_product x', 'x.idpr = a.id');
    ...
    $header_po = $this->db->get_where('tr_purchase_order_non_product', ['no_po' => $no_po])->row();
    ...
    $this->db->from('ms_department a');
    $this->db->where_in('a.id', explode(',', $header_po->id_dept));
    $get_nm_depart = $this->db->get()->result();
  ...
  public function approve_po_process()
  {
    ...
    $this->db->update('tr_purchase_order_non_product', ['status' => 2], ['no_po' => $post['no_po']]);
  ...
  public function reject_po_process()
  {
    ...
    $this->db->update('tr_purchase_order_non_product', ['status' => 1, 'reject_reason' => $post['reject_reason']], ['no_po' => $post['no_po']]);
