...
class Pr_model extends BF_Model
{
	...
	{
		$query = $this->db->query("SELECT MAX(no_po) as max_id FROM tr_purchase_order_non_product WHERE no_po LIKE '%P" . date('y', strtotime($tgl)) . "%'");
		$row = $query->row_array();
	...
	function BuatNomor($tgl)
	{
		...
		$blnthn = date('Y-m');
		$query = $this->db->query("SELECT MAX(no_surat) as max_id FROM tr_purchase_order_non_product WHERE month(tanggal)='$bulan' and Year(tanggal)='$tahun'");
		$row = $query->row_array();
		$thn = date('T');
		$max_id = $row['max_id'];
