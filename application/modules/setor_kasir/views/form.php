                            ...
                            <div class="col-md-8">
                                <select class="form-control select2" name="id_sales" id="id_sales">
                                    ...
                                    <?php foreach ($sales as $s) : ?>
                                        <option value="<?= $s->id_user ?>" data-nama="<?= $s->nm_lengkap ?>"><?= $s->nm_lengkap ?></option>
                                    <?php endforeach; ?>
                                </select>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#selectPenerimaan').on('click', function() {
            ...
            if (!sales) {
                swal("Error", "Silahkan pilih Sales terlebih dahulu", "warning");
                return;
            }

            $.ajax({
                url: siteurl + 'setor_kasir/get_penerimaan',
                type: 'POST',
                data: {
                    user_id: sales
                }, // kirim ID user yang dipilih
    ...
    function generateJurnal() {
        ...
        <tr>
            ...
            <td><textarea name="keterangan[]" class="form-control" readonly>Setoran Sales ke Kas Penjualan Cirebon</textarea></td>
    ...
    function loadSisaPiutangSebelumnya() {
        const user_id = $('#id_sales').val();

        if (!user_id) {
            console.warn('Sales belum dipilih.');
            return;
        }
